...
def preprocess_and_extract_features_mne_with_timestamps(file_name):
    ...
    # 忽略前后60s的数据
    head_seconds = 60
    head_samples = int(head_seconds * sfreq)
    tail_seconds = 60
    tail_samples = int(tail_seconds * sfreq)

    # 计算有效数据的起始和结束位置
    valid_start = head_samples
    valid_end = len(raw.times) - tail_samples
    total_windows = (valid_end - valid_start) // window_samples

    for start in tqdm(range(valid_start, valid_end, window_samples), total=total_windows, desc="Processing windows"):
